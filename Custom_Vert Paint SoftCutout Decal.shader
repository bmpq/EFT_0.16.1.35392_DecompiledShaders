Shader "Custom/Vert Paint SoftCutout Decal" {
	Properties {
        ...
        _Color0 ("Main Color 0", Color) = (1,1,1,1)
		...
		[Space(30)] [Header(________________________________________________________________________________________________________________)] 
        _Color1 ("Main Color 0", Color) = (1,1,1,1)
		...
		[Space(30)] [Header(________________________________________________________________________________________________________________)] 
        _Color2 ("Main Color 0", Color) = (1,1,1,1)
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp9;
                float4 tmp10;
                tmp0.x = tex2D(_Heights, inp.texcoord4.xy);
                tmp0.y = tex2D(_Heights, inp.texcoord4.zw);
                tmp0.z = tex2D(_Heights, inp.texcoord5.xy);
                tmp0.w = tex2D(_Heights, inp.texcoord5.zw);
                ...
                tmp3.xyz = tex2D(_BumpMap0, inp.texcoord4.xy);
                tmp4.xyz = tex2D(_BumpMap1, inp.texcoord4.zw);
                tmp4.xyz = tmp0.yyy * tmp4.xyz;
                tmp3.xyz = tmp3.xyz * tmp0.xxx + tmp4.xyz;
                tmp4.xyz = tex2D(_BumpMap2, inp.texcoord5.xy);
                tmp3.yzw = tmp4.xyz * tmp0.zzz + tmp3.xyz;
                ...
                o.sv_target = tmp0.wwww ? tmp3 : tmp1;
                o.sv_target1 = tmp0.wwww ? tmp4 : tmp2;
