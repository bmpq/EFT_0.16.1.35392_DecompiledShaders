	...
	SubShader {
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                tmp1.xyz = max(tmp1.xyz, _Ambient.xxx);
                tmp2.yzw = tmp1.xyz;
                tmp0.w = 0.0;
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp1;
                float4 tmp2;
                tmp0.xy = tex2D(_NoiseTex, inp.texcoord.zw);
                tmp0.zw = tex2D(_NoiseTex, inp.texcoord1.xy);
                tmp0.xy = tmp0.zw + tmp0.xy;
                tmp0.zw = tex2D(_NoiseTex, inp.texcoord1.zw);
                tmp0.xy = tmp0.zw + tmp0.xy;
                ...
                tmp2.xyz = -tmp1.xyz * tmp0.xyz + inp.texcoord5.xyz;
                tmp0 = tmp0 * tmp1;
                tmp1.x = inp.texcoord5.w * 0.125;
                o.sv_target.xyz = tmp1.xxx * tmp2.xyz + tmp0.xyz;
