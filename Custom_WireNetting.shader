	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord1.xy = v.texcoord.xy * _TransparentTex_ST.xy + _TransparentTex_ST.zw;
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.xyw = inp.texcoord2.xyz * tmp1.xxx + tmp2.xyz;
                tmp1.xyz = inp.texcoord4.xyz * tmp1.zzz + tmp1.xyw;
                tmp2 = tex2D(_MainTex, inp.texcoord.xy);
                tmp2.xyz = tmp2.xyz * _Color.xyz;
                tmp3 = tex2D(_TransparentTex, inp.texcoord1.xy);
                ...
                tmp4.xyz = -tmp1.xyz * _EFT_Ambient.xyz + inp.texcoord5.xyz;
                tmp2.xyz = inp.texcoord5.www * tmp4.xyz + tmp2.xyz;
                tmp0.x = _ThermalVisionOn > 0.0;
                tmp4.xyz = tmp1.xyz * float3(0.3, 0.3, 0.3);
                tmp4.xyz = max(tmp4.xyz, float3(0.0, 0.0, 0.0));
                tmp4.xyz = min(tmp4.xyz, float3(0.2, 0.2, 0.2));
                tmp1.xyz = tmp0.xxx ? tmp4.xyz : tmp1.xyz;
                tmp2.xyz = tmp0.xxx ? tmp4.xyz : tmp2.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2.xyz = tmp1.xyz * float3(0.3, 0.3, 0.3);
                tmp2.xyz = max(tmp2.xyz, float3(0.0, 0.0, 0.0));
                tmp2.xyz = min(tmp2.xyz, float3(0.2, 0.2, 0.2));
                tmp1.xyz = tmp0.www ? tmp2.xyz : tmp1.xyz;
                tmp2.xyz = inp.texcoord5.yyy * unity_WorldToLight._m01_m11_m21;
                tmp2.xyz = unity_WorldToLight._m00_m10_m20 * inp.texcoord5.xxx + tmp2.xyz;
                tmp2.xyz = unity_WorldToLight._m02_m12_m22 * inp.texcoord5.zzz + tmp2.xyz;
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord.zw = v.texcoord.xy * _TransparentTex_ST.xy + _TransparentTex_ST.zw;
			...
			fout frag(v2f inp)
			{
                ...
                tmp4 = tex2D(_TransparentTex, inp.texcoord.zw);
                tmp1.w = tmp4.x * _Color.w;
                tmp1.w = tmp1.w * _Glossness;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp3 = tex2D(_TransparentTex, inp.texcoord.zw);
                tmp1.w = tmp3.x * _Color.w;
                tmp1.w = tmp1.w * _Glossness;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1 = tex2D(_TransparentTex, inp.texcoord.zw);
                tmp0.y = tmp1.x * _Color.w;
                tmp0.y = tmp0.y * _Glossness;
                tmp0.x = tmp0.y * tmp0.x;
                tmp0.x = tmp0.x * 0.5;
                tmp0.yzw = tmp0.xxx * tmp2.xyz;
                tmp1.x = _ThermalVisionOn > 0.0;
                tmp0.yzw = tmp1.xxx ? float3(0.0, 0.0, 0.0) : tmp0.yzw;
