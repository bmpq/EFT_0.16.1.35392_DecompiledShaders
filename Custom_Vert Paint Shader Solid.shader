	...
	SubShader {
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2.xyz = tex2D(_BumpMap0, inp.texcoord4.xy);
                tmp3.xyz = tex2D(_BumpMap1, inp.texcoord4.zw);
                tmp3.xyz = tmp0.yyy * tmp3.xyz;
                tmp2.xyz = tmp2.xyz * tmp0.xxx + tmp3.xyz;
                tmp3.xyz = tex2D(_BumpMap2, inp.texcoord5.xy);
                tmp2.yzw = tmp3.xyz * tmp0.zzz + tmp2.xyz;
